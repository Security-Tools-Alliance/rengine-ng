...
class TestStartScanViews(BaseTestCase):
    ...
    @override_settings(CELERY_TASK_ALWAYS_EAGER=True)
    @patch("reNgine.secator.service.start_secator_scan")
    def test_start_scan_expert_mode_true_values(self, mock_start_scan):
        """Test start_scan_ui with different true values for expert_mode."""
        mock_start_scan.return_value = {"status": True, "scan_id": 1}
        base_data = {
        ...
        for expert_mode_value in ["true", "on", "1", "True", "On", "TRUE", "ON"]:
            with self.subTest(expert_mode_value=expert_mode_value):
                data = base_data.copy()
                data["expert_mode"] = expert_mode_value
                response = self.client.post(
                    reverse(
                        "start_scan",
                        kwargs={
                ...
                self.assertTrue(call_kwargs["expert_mode"], f"expert_mode should be True for '{expert_mode_value}'")
                mock_start_scan.reset_mock()

    @override_settings(CELERY_TASK_ALWAYS_EAGER=True)
    @patch("reNgine.secator.service.start_secator_scan")
    def test_start_scan_expert_mode_false_values(self, mock_start_scan):
        """Test start_scan_ui with different false values for expert_mode."""
        mock_start_scan.return_value = {"status": True, "scan_id": 1}
        base_data = {
            "execution_mode": "workflow",
            "workflow_id": "1",
        ...
        for expert_mode_value in ["false", "off", "0", "False", "Off", "FALSE", "OFF", ""]:
            with self.subTest(expert_mode_value=expert_mode_value):
                data = base_data.copy()
                data["expert_mode"] = expert_mode_value
                response = self.client.post(
                ...
                mock_start_scan.assert_called()
                call_kwargs = mock_start_scan.call_args.kwargs
                self.assertFalse(call_kwargs["expert_mode"], f"expert_mode should be False for '{expert_mode_value}'")
                mock_start_scan.reset_mock()

    @override_settings(CELERY_TASK_ALWAYS_EAGER=True)
    @patch("reNgine.secator.service.start_secator_scan")
    def test_start_scan_expert_mode_missing(self, mock_start_scan):
        """Test start_scan_ui with missing expert_mode parameter."""
        mock_start_scan.return_value = {"status": True, "scan_id": 1}
        data = {
            "execution_mode": "workflow",
            "workflow_id": "1",
        ...
        mock_start_scan.assert_called()
        call_kwargs = mock_start_scan.call_args.kwargs
        self.assertFalse(call_kwargs["expert_mode"], "expert_mode should default to False when missing")
