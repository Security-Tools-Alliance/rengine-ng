        ...
        def post_deletion_cleanup():
            ...
            for ip in ips_to_check:
                ...
                if not Subdomain.objects.filter(ip_addresses=ip).exists():
                    logger.warning(
                        f"Deleting orphaned IP {ip.address} as it is orphaned after deletion of subdomain {instance.name} (ID: {instance.id})"
                    )
                    ip.delete()
                else:
            ...
            with transaction.atomic():
                ...
                for ip in removed_ips:
                    if not Subdomain.objects.filter(ip_addresses=ip).exists():
                        ...
                        logger.warning(
                            "Deleting orphaned IP", extra={"ip_address": cleaned_ip, "subdomain": sanitized_subdomain}
                        )
                        ip.delete()
        except Exception as e:
