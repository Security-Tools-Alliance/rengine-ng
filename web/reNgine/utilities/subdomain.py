...
def get_new_added_subdomain(
    scan_id: int, domain_id: int, db_interface: Optional[DatabaseInterface] = None
) -> List[DatabaseRecord]:
    """
...
def _get_interesting_lookup_model(db_interface: Optional[DatabaseInterface] = None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        return {"url_lookup": True, "title_lookup": True, "condition_200_http_lookup": True}
    except Exception:
        return None
...
def get_subdomains_by_domain(
    domain_id: int, scan_history_id: Optional[int] = None, db_interface: Optional[DatabaseInterface] = None
) -> List[DatabaseRecord]:
    """
...
def get_subdomains_by_scan(
    scan_history_id: int, domain_id: Optional[int] = None, db_interface: Optional[DatabaseInterface] = None
) -> List[DatabaseRecord]:
    """
