...
class DatabaseProcessor:
    ...
    def save_endpoint(
        self, http_url: str, subdomain_id: Optional[int] = None, scan_history_id: Optional[int] = None, **kwargs
    ...
    def save_endpoints_batch(self, endpoints_data: List[Dict[str, Any]], batch_id: str = "batch") -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "saved_count": 0, "skipped_count": 0, "errors": [str(e)]}

    def save_subdomain(
        self, name: str, scan_history_id: Optional[int] = None, **kwargs
    ) -> Tuple[Optional[DatabaseRecord], bool]:
        ...
        try:
            subdomain_data = {"name": name, "scan_history_id": scan_history_id, **kwargs}
    ...
    def save_vulnerability(
        self, name: str, http_url: str, scan_history_id: Optional[int] = None, **kwargs
    ) -> Tuple[Optional[DatabaseRecord], bool]:
        ...
        try:
            vulnerability_data = {"name": name, "http_url": http_url, "scan_history_id": scan_history_id, **kwargs}
...
def save_endpoint(
    ...
    **kwargs,
    ...
    return processor.save_endpoint(
        http_url=http_url, subdomain_id=subdomain_id, scan_history_id=scan_history_id, **kwargs
    )


def save_subdomain(
    name: str, scan_history_id: Optional[int] = None, db_interface: Optional[DatabaseInterface] = None, **kwargs
) -> Tuple[Optional[DatabaseRecord], bool]:
    """
    ...
    """
    processor = DatabaseProcessor(db_interface=db_interface)
    return processor.save_subdomain(name=name, scan_history_id=scan_history_id, **kwargs)


def validate_and_save_subdomain(
    subdomain_name: str, ctx: Dict[str, Any] = None, db_interface: Optional[DatabaseInterface] = None
) -> Tuple[Optional[DatabaseRecord], bool]:
    """
    ...
    try:
        ...
        existing = db_interface.filter_records(
            "subdomain", [QueryFilter("name", subdomain_name), QueryFilter("scan_history_id", scan_history_id)]
        )
        ...
        subdomain_data = {"name": subdomain_name, "scan_history_id": scan_history_id}
...
def save_vulnerability(
    ...
    **kwargs,
    ...
    """
    processor = DatabaseProcessor(db_interface=db_interface)
    return processor.save_vulnerability(name=name, http_url=http_url, scan_history_id=scan_history_id, **kwargs)
...
def save_technology(
    ...
    **kwargs,
    ...
    try:
        ...
        existing = db_interface.filter_records("technology", [QueryFilter("name", name)])
        ...
        if existing:
            ...
            technology_data = {"name": name, **kwargs}
...
def save_waf(
    name: str, manufacturer: str = "", db_interface: Optional[DatabaseInterface] = None, **kwargs
) -> Tuple[Optional[DatabaseRecord], bool]:
    """
    ...
    try:
        ...
        existing = db_interface.filter_records(
            "waf", [QueryFilter("name", name), QueryFilter("manufacturer", manufacturer)]
        )
        ...
        if existing:
            ...
            # Create new WAF
            waf_data = {"name": name, "manufacturer": manufacturer, **kwargs}
...
def validate_database_input(data: Dict[str, Any], required_fields: List[str] = None) -> Dict[str, Any]:
    """
...
def get_database_statistics(
    scan_history_id: Optional[int] = None, db_interface: Optional[DatabaseInterface] = None
) -> Dict[str, Any]:
    """
    ...
    """
    if not db_interface:
        return {"endpoints": 0, "subdomains": 0, "vulnerabilities": 0, "technologies": 0, "wafs": 0}

    try:
        if not scan_history_id:
            return {"endpoints": 0, "subdomains": 0, "vulnerabilities": 0, "technologies": 0, "wafs": 0}

        endpoints = len(db_interface.filter_records("endpoint", [QueryFilter("scan_history_id", scan_history_id)]))

        subdomains = len(db_interface.filter_records("subdomain", [QueryFilter("scan_history_id", scan_history_id)]))

        vulnerabilities = len(
            db_interface.filter_records("vulnerability", [QueryFilter("scan_history_id", scan_history_id)])
        )
...
def save_fuzzing_file(
    ...
    **kwargs,
    ...
    try:
        ...
        existing = db_interface.filter_records(
            "directory_file",
            [QueryFilter("name", name), QueryFilter("url", url), QueryFilter("http_status", http_status)],
        )
