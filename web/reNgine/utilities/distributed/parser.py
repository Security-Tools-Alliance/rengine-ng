...
from reNgine.utilities.core.file import (
...
class DistributedNmapParser(DistributedParserProcessor):
    ...
    def parse_nmap_files_batch(
        ...
        **kwargs,
        ...
        result = DistributedParserResult(data={}, status=ProcessingStatus.IN_PROGRESS, batch_id=batch_id)
    ...
    def _parse_nmap_file(
        self, xml_file: str, parse_type: str, output_dir: Optional[str] = None
    ) -> List[Dict[str, Any]]:
...
class DistributedNucleiParser(DistributedParserProcessor):
    ...
    def parse_nuclei_files_batch(
        self, nuclei_files: List[str], batch_id: str, output_dir: Optional[str] = None, **kwargs
    ) -> DistributedParserResult:
            ...
            with self.safe_execution():
                ...
                for nuclei_file in nuclei_files:
                    try:
                        ...
                        self.record_parsed_item(
                            batch_id, f"Parsed {len(file_results)} vulnerabilities from {nuclei_file}"
                        )
...
class DistributedHttpxParser(DistributedParserProcessor):
    ...
    def parse_httpx_files_batch(
        self, httpx_files: List[str], batch_id: str, output_dir: Optional[str] = None, **kwargs
    ) -> DistributedParserResult:
...
def create_distributed_nmap_parser(batch_size: int = 15, worker_timeout: int = 300, **kwargs) -> DistributedNmapParser:
...
def parse_nuclei_files_distributed(
    ...
    **kwargs,
    ...
    # Create batch tasks
    batch_tasks = create_batch_tasks(
        nuclei_files, parser.parse_nuclei_files_batch, parser.config.batch_size, output_dir=output_dir, **kwargs
    )
...
def parse_httpx_files_distributed(
    httpx_files: List[str], parser: Optional[DistributedHttpxParser] = None, output_dir: Optional[str] = None, **kwargs
) -> Dict[str, Any]:
    ...
    # Create batch tasks
    batch_tasks = create_batch_tasks(
        httpx_files, parser.parse_httpx_files_batch, parser.config.batch_size, output_dir=output_dir, **kwargs
    )
