...
class RengineTask(Task):
    ...
    def __call__(self, *args, **kwargs):
        ...
        context = TaskContext(ctx=ctx, request=self.request, task_name=task_name, description=description)
        ...
        if not context.filename:
            context.filename = get_output_file_name(context.scan_id, context.subscan_id, f"{context.task_name}.txt")
        ...
        if RENGINE_RECORD_ENABLED:
            if context.engine:
                ...
                if (
                    context.track
                    and context.task_name not in context.engine.tasks
                    and dependent_tasks.get(context.task_name) not in context.engine.tasks
                    and context.task_name.lower() not in exempted_tasks
                ):
                    logger.debug(
                        f'Task {context.task_name} is not part of engine "{context.engine.engine_name}" tasks. Skipping.'
                    )
                    return False
            ...
            if context.track:
                ...
                if context.domain:
                    logger.warning(
                        f"Task {task_identifier} for {context.subdomain.name if context.subdomain else context.domain.name} is RUNNING"
                    )
                else:
        ...
        if RENGINE_CACHE_ENABLED:
            ...
            if cached_result and cached_result != b"null":
                ...
                if RENGINE_RECORD_ENABLED and context.track:
                    ...
                    if context.domain:
                        logger.warning(
                            f"Task {task_identifier} for {context.subdomain.name if context.subdomain else context.domain.name} status is SUCCESS (CACHED)"
                        )
                    else:
        ...
        try:
            ...
            context.output_path = get_traceback_path(
                context.task_name, context.results_dir, context.scan_id, context.subscan_id
            )
