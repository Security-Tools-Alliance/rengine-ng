...
def port_scan(self, hosts=None, ctx=None, description=None):
    ...
    if hosts:
        ...
        hosts = get_subdomains(write_filepath=input_file, exclude_subdomains=exclude_subdomains, ctx=ctx)
    ...
    for line in stream_command(
        ...
        # Grab subdomain
        subdomain = Subdomain.objects.filter(name=host, target_domain=self.domain, scan_history=self.scan).first()
        ...
        if is_web_port:
            if port_number == 80:
                ...
                endpoint, created = save_endpoint(http_url, ctx=ctx, subdomain=subdomain, is_default=True)
                if endpoint:
                ...
                for scheme in ["http", "https"]:
                    ...
                    endpoint, created = save_endpoint(http_url, ctx=ctx, subdomain=subdomain, is_default=False)
                    if endpoint:
        ...
        if is_web_port:
            web_services.extend(
                [
                    {"host": ip_address, "port": port_number, "scheme": ep["scheme"], "url": ep["url"]}
        ...
        port, created = Port.objects.update_or_create(
            ...
            defaults={
                ...
                "description": f"Web service on port {port_number}"
                if is_web_port
                else f"Service on port {port_number}",
        ...
        if created:
            logger.warning(
                f"Found opened port {port_number} on {ip_address} ({host}) - Created {len(endpoints_created)} endpoint(s)"
            )
        else:
...
def nmap(
    ...
    # Run cmd
    run_command(
        nmap_cmd, shell=True, history_file=self.history_file, scan_id=self.scan_id, activity_id=self.activity_id
    )
