...
def pre_crawl(self, ctx={}, description=None):
    ...
    existing_subdomains = Subdomain.objects.filter(
        target_domain_id=domain_id, scan_history_id=ctx.get("scan_history_id")
    )
    ...
    if urls_to_crawl:
        ...
        logger.info(
            f"Found {total_subdomains} subdomains and {total_ips_resolved} discovered IPs (no endpoints to test)"
        )
