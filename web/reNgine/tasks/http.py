...
class HTTPCrawlProcessor:
    ...
    def process_http_crawl_batch(self, urls: List[str], ctx: Dict[str, Any], batch_id: str, **kwargs) -> Dict[str, Any]:
        """Process a batch of URLs for HTTP crawling"""
        try:
            # Validate URLs using distributed URL processor
            url_result = self.url_processor.validate_urls_batch(urls, batch_id, **kwargs)

            if not url_result.is_successful:
                return {"success": False, "error": f"URL validation failed: {url_result.errors}", "batch_id": batch_id}

            # Process valid URLs
            valid_urls = url_result.data.get("valid_urls", [])
            if not valid_urls:
                return {"success": True, "message": "No valid URLs to process", "batch_id": batch_id}

            # Execute HTTP crawling commands
            crawl_results = self._execute_http_crawl_commands(valid_urls, ctx, batch_id, **kwargs)

            # Parse results using distributed parser
            parsed_results = self._parse_http_crawl_results(crawl_results, ctx, batch_id, **kwargs)

            # Save endpoints using distributed database processor
            saved_results = self._save_http_crawl_results(parsed_results, ctx, batch_id, **kwargs)
    ...
    def _execute_http_crawl_commands(
        self, urls: List[str], ctx: Dict[str, Any], batch_id: str, **kwargs
    ) -> List[Dict[str, Any]]:
        ...
        for url in urls:
            ...
            commands.append({"command": command, "url": url, "output_file": f"/tmp/httpx_{batch_id}_{hash(url)}.json"})
    ...
    def _parse_http_crawl_results(
        self, crawl_results: List[Dict[str, Any]], ctx: Dict[str, Any], batch_id: str, **kwargs
    ) -> List[Dict[str, Any]]:
        ...
        parse_result = self.httpx_parser.parse_httpx_files_batch(output_files, batch_id, **kwargs)
    ...
    def _save_http_crawl_results(
        self, parsed_results: List[Dict[str, Any]], ctx: Dict[str, Any], batch_id: str, **kwargs
    ) -> Dict[str, Any]:
        ...
        save_result = self.endpoint_processor.save_endpoints_batch(endpoint_data, batch_id, **kwargs)
...
def http_crawl_distributed(self, urls=None, ctx=None, description=None, **kwargs):
    """
    ...
    try:
        ...
        for i in range(0, len(urls), batch_size):
            ...
            batch_result = processor.process_http_crawl_batch(batch_urls, ctx, batch_id, **kwargs)
        ...
        logger.info(
            f"Distributed HTTP crawl completed: {total_processed} URLs processed, {total_saved} endpoints saved"
        )
        ...
        return {"success": False, "error": str(e), "total_urls": len(urls)}


@app.task(name="pre_crawl_distributed", queue="io_queue", base=RengineTask, bind=True)
def pre_crawl_distributed(self, subdomain_id=None, ctx=None, description=None, **kwargs):
    """
    ...
    try:
        ...
        if not urls:
            return {"success": True, "message": "No URLs generated for pre-crawl", "subdomain_id": subdomain_id}
        ...
        for i in range(0, len(urls), batch_size):
            ...
            command_result = command_processor.execute_commands_batch(commands, batch_id, **kwargs)
        ...
        if results:
            ...
            save_result = endpoint_processor.save_endpoints_batch(endpoint_data, f"pre_crawl_{subdomain_id}", **kwargs)
    ...
    if ctx.get("all_ports", False):
        ...
        for port in ports:
            if port in [80, 443]:
                ...
                # Non-standard ports
                urls.extend([f"http://{subdomain.name}:{port}", f"https://{subdomain.name}:{port}"])
...
def http_crawl(self, urls=None, ctx=None, description=None, **kwargs):
    """
    ...
    return http_crawl_distributed.delay(urls=urls, ctx=ctx, description=description, **kwargs).get()
    ...
    **kwargs,
    ...
    try:
        ...
        return http_crawl_distributed.delay(
            urls=urls, ctx=ctx, description=f"Batch {batch_id} - {phase}", **kwargs
        ).get()
    except Exception as e:
        logger.error(f"Error in http_crawl_batch {batch_id}: {e}")
        return {"success": False, "error": str(e), "batch_id": batch_id, "processed_urls": 0, "discovered_endpoints": 0}
...
def http_crawl_orchestrator(
    ...
    **kwargs,
    ...
    try:
        # Use the distributed HTTP crawl system
        return http_crawl_distributed.delay(urls=urls, ctx=ctx, description=f"Orchestrator - {phase}", **kwargs).get()
    except Exception as e:
        logger.error(f"Error in http_crawl_orchestrator {phase}: {e}")
        return {"success": False, "error": str(e), "phase": phase, "processed_urls": 0, "discovered_endpoints": 0}


@app.task(name="http_crawl_coordinator", queue="io_queue", base=RengineTask, bind=True)
def http_crawl_coordinator(
    self, ctx: Dict[str, Any], phase: str, description: Optional[str] = None, **kwargs
) -> Dict[str, Any]:
    """
...
def pre_crawl(self, ctx={}, description=None):
    ...
    result = http_crawl_coordinator.delay(
        ctx=ctx, phase=CrawlPhase.PRE_CRAWL, description=description or "Pre-crawl endpoints"
    )
...
def intermediate_crawl(self, ctx={}, description=None):
    ...
    result = http_crawl_coordinator.delay(
        ctx=ctx, phase=CrawlPhase.INTERMEDIATE_CRAWL, description=description or "Intermediate crawl endpoints"
    )
...
def post_crawl(self, ctx={}, description=None):
    ...
    result = http_crawl_coordinator.delay(
        ctx=ctx, phase=CrawlPhase.POST_CRAWL, description=description or "Post-crawl verification"
    )
