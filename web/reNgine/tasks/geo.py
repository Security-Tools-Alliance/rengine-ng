...
from reNgine.utilities.core.validation import is_valid_ip
...
class GeolocationProcessor:
    ...
    def geolocate_ips_batch(self, ip_addresses: List[str], batch_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.network_processor.geolocate_ips_batch(valid_ips, batch_id, **kwargs)
            ...
            return {"success": False, "batch_id": batch_id, "error": str(e), "total_ips": len(ip_addresses)}

    def geolocate_single_ip(self, ip_address: str, **kwargs) -> Dict[str, Any]:
        """Geolocate a single IP address"""
        try:
            # Validate IP
            if not is_valid_ip(ip_address):
                return {"success": False, "ip_address": ip_address, "error": "Invalid IP address"}
            ...
            if not success:
                return {"success": False, "ip_address": ip_address, "error": error}
            ...
            if country_iso and country_name:
                ...
                return {"success": False, "ip_address": ip_address, "error": "No geolocation data found"}
    ...
    def update_ip_geolocation(
        self, ip_address: str, country_iso: str, country_name: str, ip_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """Update IP geolocation in database using distributed database processor"""
        try:
            # Create or get CountryISO object
            geo_object, created = CountryISO.objects.get_or_create(iso=country_iso, name=country_name)
...
def geo_localize(host, ip_id=None):
    ...
    # Update database
    update_result = processor.update_ip_geolocation(host, result["country_iso"], result["country_name"], ip_id)

    if update_result["success"]:
        geo_json = {"iso": result["country_iso"], "name": result["country_name"]}
        return geo_json
    else:
...
def geo_localize_batch(ip_addresses):
            ...
            for geo_data in result.get("geolocated_ips", []):
                try:
                    ...
                    update_result = processor.update_ip_geolocation(
                        geo_data["ip_address"], geo_data["country_iso"], geo_data["country_name"], ip_obj.id
                    )

                    if update_result["success"]:
                        logger.debug(
                            f"Successfully geolocalized {geo_data['ip_address']} -> {geo_data['country_name']}"
                        )
                        total_success += 1
                    else:
                        logger.error(
                            f"Failed to update geolocation for {geo_data['ip_address']}: {update_result.get('error', 'Unknown error')}"
                        )
                        total_failed += 1
    ...
    result = {"success": total_success, "failed": total_failed, "skipped": total_skipped, "total": len(ip_addresses)}
...
def geolocate_ips_distributed(ip_addresses: List[str], **kwargs) -> Dict[str, Any]:
    """
    Geolocate IP addresses using distributed processing.
    """
    processor = GeolocationProcessor()
    return processor.geolocate_ips_batch(ip_addresses, "distributed_geolocation", **kwargs)


def geolocate_single_ip_distributed(ip_address: str, **kwargs) -> Dict[str, Any]:
    """
...
def get_country_statistics(geolocated_ips: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if not geolocated_ips:
        return {"total_countries": 0, "country_distribution": {}, "top_countries": []}
    ...
    top_countries = sorted(country_counts.items(), key=lambda x: x[1], reverse=True)[:10]  # Top 10 countries
...
def filter_geolocation_results(
    results: Dict[str, Any], country_filter: Optional[List[str]] = None, exclude_private: bool = True
) -> Dict[str, Any]:
    """
    ...
    if "geolocated_ips" in results:
        ...
        # Filter by country
        if country_filter:
            geolocated_ips = [ip for ip in geolocated_ips if ip.get("country_name") in country_filter]
