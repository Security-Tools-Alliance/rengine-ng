...
class OSINTProcessor:
    ...
    def process_osint_scan_batch(self, domain_id: int, ctx: Dict[str, Any], batch_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            scan_results = self._execute_osint_commands(domain, ctx, batch_id, **kwargs)

            # Parse OSINT results
            parsed_results = self._parse_osint_results(scan_results, ctx, batch_id, **kwargs)

            # Save OSINT results
            saved_results = self._save_osint_results(parsed_results, ctx, batch_id, **kwargs)
    ...
    def _execute_osint_commands(
        self, domain: Domain, ctx: Dict[str, Any], batch_id: str, **kwargs
    ) -> List[Dict[str, Any]]:
    ...
    def _parse_osint_results(
        ...
        """Parse OSINT results"""
        parsed_results = {"subdomains": [], "emails": [], "employees": [], "documents": []}
    ...
    def _save_osint_results(
        ...
        for subdomain_name in parsed_results.get("subdomains", []):
            subdomain_data.append(
                {"name": subdomain_name, "scan_history": ctx.get("scan_history"), "domain": ctx.get("domain")}
            )
...
def osint_scan_distributed(self, domain_id=None, ctx=None, description=None, **kwargs):
    """
    ...
    try:
        ...
        # Process OSINT scan
        result = processor.process_osint_scan_batch(domain_id, ctx, f"osint_scan_{domain_id}", **kwargs)
...
def osint_scan_tool_distributed(self, tool_name=None, domain_id=None, ctx=None, description=None, **kwargs):
    """
    ...
    try:
        ...
        # Process OSINT scan
        result = processor.process_osint_scan_batch(domain_id, ctx, f"osint_scan_{tool_name}_{domain_id}", **kwargs)
        ...
        return {"success": False, "error": str(e), "tool_name": tool_name, "domain_id": domain_id}


# Legacy task wrapper for backward compatibility
@app.task(name="osint_scan", queue="io_queue", base=RengineTask, bind=True)
def osint_scan(self, domain_id=None, ctx=None, description=None, **kwargs):
    """
    ...
    return osint_scan_distributed.delay(domain_id=domain_id, ctx=ctx, description=description, **kwargs).get()


@app.task(name="osint_scan_batch", queue="io_queue", base=RengineTask, bind=True)
def osint_scan_batch(self, domain_id=None, ctx=None, description=None, **kwargs):
    """
    ...
    try:
        ...
        result = osint_scan_distributed.delay(
            domain_id=domain_id, ctx=ctx, description=description or "OSINT scan batch", **kwargs
        )
        ...
        return {"success": False, "error": str(e), "scanned_domains": 0, "osint_results": 0}


@app.task(name="osint_scan_orchestrator", queue="orchestrator_queue", base=RengineTask, bind=True)
def osint_scan_orchestrator(self, domain_id=None, ctx=None, description=None, **kwargs):
    """
    ...
    try:
        # Use the distributed OSINT scan system
        result = osint_scan_distributed.delay(
            domain_id=domain_id, ctx=ctx, description=description or "OSINT scan orchestration", **kwargs
        )
