...
class LLMProcessor:
    ...
    def generate_batch_vulnerability_reports(
        self, vulnerability_ids: List[int], force_regenerate: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for vuln_id in vulnerability_ids:
                try:
                    result = self.generate_vulnerability_report(
                        vulnerability_id=vuln_id, force_regenerate=force_regenerate
                    )
                    ...
                    failed += 1
                    results.append({"vulnerability_id": vuln_id, "result": {"status": False, "error": str(e)}})
            ...
            return {"success": False, "error": str(e), "total_vulnerabilities": len(vulnerability_ids)}

    def convert_markdown_to_html_distributed(self, markdown_text: str) -> str:
...
def llm_vulnerability_report(
    ...
    return processor.generate_vulnerability_report(
        vulnerability_id=vulnerability_id, vuln_tuple=vuln_tuple, force_regenerate=force_regenerate
    )


@app.task(name="llm_vulnerability_report_batch", bind=False, queue="cpu_queue")
def llm_vulnerability_report_batch(vulnerability_ids: List[int], force_regenerate: bool = False):
    """
    ...
    return processor.generate_batch_vulnerability_reports(
        vulnerability_ids=vulnerability_ids, force_regenerate=force_regenerate
    )
...
def generate_vulnerability_report_distributed(
    vulnerability_id: Optional[int] = None, vuln_tuple: Optional[Tuple[str, str]] = None, force_regenerate: bool = False
) -> Dict[str, Any]:
    """
...
def generate_batch_vulnerability_reports_distributed(
    vulnerability_ids: List[int], force_regenerate: bool = False
) -> Dict[str, Any]:
    """
...
def convert_markdown_to_html_distributed(markdown_text: str) -> str:
    """
...
def validate_llm_input(
    vulnerability_id: Optional[int] = None, vuln_tuple: Optional[Tuple[str, str]] = None
) -> Dict[str, Any]:
    """
...
def filter_llm_results(
    results: Dict[str, Any], status_filter: Optional[str] = None, model_filter: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    filtered_results["successful_reports"] = sum(
        1 for r in filtered_results["results"] if r.get("result", {}).get("status")
    )
