...
class ScanOrchestrationProcessor:
    ...
    def process_scan_orchestration(
        self, scan_history_id: int, domain_id: int, ctx: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            orchestration_result = self._initialize_scan_orchestration(scan_history, domain, ctx, **kwargs)
            ...
            scan_results = self._execute_distributed_scan_phases(scan_history, domain, ctx, **kwargs)

            # Finalize scan
            return self._finalize_scan_orchestration(scan_history, domain, scan_results, **kwargs)
    ...
    def _initialize_scan_orchestration(
        self, scan_history: ScanHistory, domain: Domain, ctx: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
        """Initialize scan orchestration"""
        try:
            ...
            if scan_context["imported_subdomains"]:
                save_result = self.subdomain_processor.save_subdomains_batch(
                    scan_context["imported_subdomains"], f"imported_subdomains_{scan_history.id}", **kwargs
                )

                if not save_result.is_successful:
                    logger.warning(f"Failed to save imported subdomains: {save_result.errors}")

            return {"success": True, "scan_context": scan_context}
    ...
    def _finalize_scan_orchestration(
        self, scan_history: ScanHistory, domain: Domain, scan_results: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                send_scan_notification_distributed.delay(scan_history_id=scan_history.id, status="completed")
            except Exception as e:
            ...
            try:
                ...
                generate_scan_report_distributed.delay(scan_history_id=scan_history.id)
            except Exception as e:
...
def initiate_scan_distributed(
    ...
    **kwargs,
    ...
    try:
        ...
        result = processor.process_scan_orchestration(scan_history_id, domain_id, scan_context, **kwargs)
...
def schedule_scan_distributed(domain_id, engine_id=None, scan_type=SCHEDULED_SCAN, scheduled_time=None, **kwargs):
    """
    ...
    try:
        ...
        scan_history = create_scan_object(
            domain_id=domain_id, engine_id=engine_id, scan_type=scan_type, scheduled_time=scheduled_time, **kwargs
        )

        if not scan_history:
            return {"success": False, "error": "Failed to create scan history"}
        ...
        if scheduled_time:
            ...
            initiate_scan_distributed.apply_async(
                ...
                kwargs={"engine_id": engine_id, "scan_type": scan_type, **kwargs},
                eta=scheduled_time,
            )
        else:
            # Execute immediately
            initiate_scan_distributed.delay(
                scan_history.id, domain_id, engine_id=engine_id, scan_type=scan_type, **kwargs
            )
...
def scan_orchestrator(scan_history_id, domain_id, engine_id=None, scan_type=LIVE_SCAN, **kwargs):
    """
    ...
    try:
        ...
        result = initiate_scan_distributed.delay(
            scan_history_id, domain_id, engine_id=engine_id, scan_type=scan_type, **kwargs
        )
        ...
        logger.error(f"Scan orchestration failed for scan {scan_history_id}: {e}")
        return {"success": False, "error": str(e), "scan_history_id": scan_history_id, "domain_id": domain_id}


@app.task(name="scan_coordinator", bind=False, queue="orchestrator_queue")
def scan_coordinator(scan_history_id, domain_id, **kwargs):
    """
    ...
    try:
        # Use the distributed scan initiation system
        result = initiate_scan_distributed.delay(scan_history_id, domain_id, **kwargs)
...
def initiate_subscan(subdomain_id, engine_id=None, scan_type=None, results_dir=None, url_filter=""):
    ...
    try:
        ...
        send_notification.delay(
            f"Subscan {scan_type} started for {subdomain.name}", scan_history_id=scan.id, subscan_id=subscan.id
        )
