...
def subdomain_discovery(self, host=None, ctx=None, description=None):
    ...
    default_subdomain_tools = [
        tool.name.lower()
        for tool in InstalledExternalTool.objects.filter(is_default=True).filter(is_subdomain_gathering=True)
    ]
    custom_subdomain_tools = [
        tool.name.lower()
        for tool in InstalledExternalTool.objects.filter(is_default=False).filter(is_subdomain_gathering=True)
    ]
    ...
    for tool in tools:
        ...
        if tool in default_subdomain_tools:
            if tool == "amass-passive":
                ...
                cmd += (
                    (" -config " + str(Path.home() / ".config" / "subfinder" / "config.yaml"))
                ...
                cmd_extract = (
                    "cut -d',' -f6 "
                    + str(Path(RENGINE_TOOL_GITHUB_PATH) / "OneForAll" / "results" / f"{host}.csv")
                    + " | tail -n +2 > "
                    + str(Path(self.results_dir) / "subdomains_oneforall.txt")
                )
            ...
            logger.warning(f'Subdomain discovery tool "{tool}" is not supported by reNgine. Skipping.')
            continue

        # Run tool
        try:
            run_command(
                cmd, shell=True, history_file=self.history_file, scan_id=self.scan_id, activity_id=self.activity_id
            )
    ...
    for line in lines:
        ...
        valid_domain = (
            bool(validators.domain(subdomain_name))
            or bool(validators.ipv4(subdomain_name))
            or bool(validators.ipv6(subdomain_name))
            or valid_url
        )
        if not valid_domain:
    ...
    for subdomain in subdomains:
        ...
        endpoint, _ = save_endpoint(http_url, ctx=ctx, is_default=True, subdomain=subdomain)
    ...
    if send_interesting and self.scan_id and self.domain_id:
        if interesting_subdomains := get_interesting_subdomains(self.scan_id, self.domain_id):
