...
from reNgine.utilities.core import is_valid_url
from reNgine.utilities.distributed import DistributedCommandBuilder, get_distributed_utilities
...
class DetectionProcessor:
    ...
    def build_wafw00f_command(self, input_file: str, output_file: str, format_type: str = "json") -> str:
    ...
    def build_cmseek_command(
        self, url: str, random_agent: bool = True, batch_mode: bool = True, follow_redirect: bool = True
    ) -> str:
    ...
    def execute_waf_detection(self, urls: List[str], input_file: str, output_file: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # Execute command
            result = self.command_processor.execute_commands_batch([command], "waf_detection", timeout=300, **kwargs)
    ...
    def execute_cms_detection(self, url: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # Execute command
            result = self.command_processor.execute_commands_batch([command], "cms_detection", timeout=600, **kwargs)

            if result.is_successful:
                return {"success": True, "command": command, "url": url, "execution_time": result.processing_time}
            else:
                return {
                    "success": False,
                    "command": command,
            ...
            return {"success": False, "command": command, "url": url, "error": str(e), "execution_time": 0}
...
def waf_detection(self, ctx={}, description=None):
    ...
    def _execute_waf_detection(ctx, description):
        ...
        result = processor.execute_waf_detection(urls=urls, input_file=input_path, output_file=self.output_path)
...
def run_wafw00f(url):
    ...
    try:
        ...
        # Execute command
        result = processor.command_processor.execute_commands_batch([command], "single_wafw00f", timeout=300)
...
def detect_waf_distributed(urls: List[str], input_file: str, output_file: str, **kwargs) -> Dict[str, Any]:
    """
    Detect WAF using distributed processing.
    """
    processor = DetectionProcessor()
    return processor.execute_waf_detection(urls=urls, input_file=input_file, output_file=output_file, **kwargs)


def detect_cms_distributed(url: str, **kwargs) -> Dict[str, Any]:
    """
    Detect CMS using distributed processing.
    """
    processor = DetectionProcessor()
    return processor.execute_cms_detection(url=url, **kwargs)
...
def get_detection_statistics(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    """
    if not results:
        return {"total_checked": 0, "waf_detected": 0, "cms_detected": 0, "no_detection": 0}
