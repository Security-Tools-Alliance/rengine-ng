        ...
        **kwargs
        ...
        try:
            result = self.command_processor.execute_commands_batch(
                [command], "ffuf_fuzzing", timeout=600, **kwargs
            )
    ...
    def _execute_dir_file_fuzz(ctx, description):
        ...
        for url in urls:
            ...
            if stream_result["success"]:
                ...
                processed_results = processor.process_ffuf_results(
                    ffuf_results, ctx, self.scan, self.subscan
                )
...
def fuzz_directories_distributed(
    ...
    **kwargs
    ...
    return {
        "success": True,
        "total_urls": len(urls),
        "successful_fuzzes": len(results),
        "results": results
    }


def build_ffuf_command_distributed(
    wordlist_path: str,
    target_url: str,
    **kwargs
) -> str:
    ...
    """
    processor = FuzzingProcessor()
    return processor.build_ffuf_command(
        wordlist_path=wordlist_path,
        target_url=target_url,
        **kwargs
    )
