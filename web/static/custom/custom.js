...
function setUrlParam(url, key, value) {
    try {
        ...
        if (regex.test(url)) {
            return url.replace(regex, `${hasQuestion ? '?' : ''}${key}=${encodeURIComponent(value)}`);
        }
...
function render_llm_vuln_modal(data, title, endpoint_url, vuln_id){
    ...
    // Sanitize model name explicitly before interpolation
    const safeModel = data.llm_model ? DOMPurify.sanitize(String(data.llm_model), {ALLOWED_TAGS: [], ALLOWED_ATTR: []}) : null;
    const modelBadge = safeModel ? `<span class="badge bg-soft-primary text-primary mb-3 d-inline-block">Generated by ${safeModel}</span>` : '';
    const bodyHtml = `
        ${modelBadge}
        <h4>Description</h4>
