...
from scanEngine.models import EngineType, SecatorWorkflow, Wordlist
...
class TestLoadWorkflowsCommand(BaseTestCase):
    ...
    @patch("scanEngine.management.commands.load_workflows.Command._execute_secator_command")
    @patch("builtins.open", new_callable=mock_open)
    def test_load_builtin_workflows_success(self, mock_file, mock_execute):
        """
        ...
        mock_result = type(
            ...
            {
                "returncode": 0,
                "stderr": "/home/rengine/.cache/pypoetry/virtualenvs/celery-rengine-HmEJnPQT-py3.12/lib/python3.12/site-packages/secator/configs/workflows/subdomain_recon.yaml\n",
                "stdout": "",
            },
        )()
...
"""
        ...
        # Call the command
        call_command("load_workflows", "--builtin-only", "--force")

        # Verify workflow was created
        workflow = SecatorWorkflow.objects.get(alias="subdomain_recon")
        self.assertEqual(workflow.name, "Subdomain Recon")
    ...
    @patch("scanEngine.management.commands.load_workflows.Command._execute_secator_command")
    def test_load_builtin_workflows_secator_failure(self, mock_execute):
        """
        Test handling of secator command failure.
        """
        # Mock the secator command failure
        mock_result = type("MockResult", (), {"returncode": 1, "stderr": "Command failed", "stdout": ""})()
        mock_execute.return_value = mock_result
    ...
    @patch("scanEngine.management.commands.load_workflows.Command._execute_secator_command")
    @patch("builtins.open", new_callable=mock_open)
    def test_load_builtin_workflows_file_not_found(self, mock_file, mock_execute):
        """
    ...
    @patch("scanEngine.management.commands.load_workflows.Command._execute_secator_command")
    @patch("builtins.open", new_callable=mock_open)
    def test_load_builtin_workflows_invalid_yaml(self, mock_file, mock_execute):
        """
    ...
    @patch("scanEngine.management.commands.load_workflows.Command._execute_secator_command")
    @patch("builtins.open", new_callable=mock_open)
    def test_load_builtin_workflows_update_existing(self, mock_file, mock_execute):
        """
...
"""
    ...
    def test_determine_scan_type_internal(self):
        ...
        """
        from scanEngine.management.commands.load_workflows import Command

        command = Command()

        # Test with internal keywords
        workflow_data = {
            "tasks": {"nmap": {"description": "Port scan"}, "naabu": {"description": "Port discovery"}},
            "description": "Network reconnaissance",
        }

        scan_type = command._determine_scan_type(workflow_data, "host_recon")
        self.assertEqual(scan_type, "internal")
    ...
    def test_determine_scan_type_internet(self):
        ...
        workflow_data = {
            "tasks": {"subfinder": {"description": "Subdomain discovery"}, "httpx": {"description": "HTTP probe"}},
            "description": "Web reconnaissance",
        }

        scan_type = command._determine_scan_type(workflow_data, "subdomain_recon")
        self.assertEqual(scan_type, "internet")

    def test_extract_workflow_file_path_truncated(self):
        """
        Test extraction of workflow file path with truncated output.
        """
        from scanEngine.management.commands.load_workflows import Command

        command = Command()

        lines = ["/home/rengine/.cache/pypoetry/virtualenvs/celery-rengine-HmEJnPQT-py3.12/lib/pyt"]
        file_path = command._extract_workflow_file_path(lines, "subdomain_recon")

        expected_path = "/home/rengine/.cache/pypoetry/virtualenvs/celery-rengine-HmEJnPQT-py3.12/lib/python3.12/site-packages/secator/configs/workflows/subdomain_recon.yaml"
        self.assertEqual(file_path, expected_path)

    def test_extract_workflow_file_path_fallback(self):
        """
        Test extraction of workflow file path with fallback.
        """
        from scanEngine.management.commands.load_workflows import Command
